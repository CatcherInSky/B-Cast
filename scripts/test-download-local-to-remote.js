...
/**
 * 测试脚本：本地下载到线上R2
 * 从本地D1读取待下载列表，下载到本地临时目录，上传到线上R2，更新本地D1状态
 * 
 * 使用方式：
 *   pnpm test:download:local-to-remote [--limit <number>]
 * 
 * 前置条件：
...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--limit' && i + 1 < args.length) {
      ...
      console.log(`
用法: node test-download-local-to-remote.js [选项]
...
示例:
  node test-download-local-to-remote.js --limit 3
`);
      process.exit(0);
    }
  }
...
async function main() {
  ...
  // 获取待下载列表
  const items = await getPendingDownloads(workerUrl, args.limit || 1);
